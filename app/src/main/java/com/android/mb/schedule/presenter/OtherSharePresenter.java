...
public class OtherSharePresenter extends BaseMvpPresenter<IOtherShareView> implements IOtherSharePresenter {
    ...
    public void getDataFromLocal(Map<String, Object> requestMap) {
        ...
        Observable  observable = scheduleDao.queryBuilder()
                .where(ScheduleDao.Properties.Share.like(String.valueOf(uid))).orderDesc(ScheduleDao.Properties.Time_s).orderDesc(ScheduleDao.Properties.Id).limit(20).offset(offset).rx().list();
